...
export default function CommercialLanding() {
  ...
  const [showClienteHogar, setShowClienteHogar] = useState(false);
  const clienteHogarRef = useRef<HTMLDivElement>(null);
  const { toast } = useToast();

  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (clienteHogarRef.current && !clienteHogarRef.current.contains(e.target as Node)) {
        setShowClienteHogar(false);
      }
    }
    if (showClienteHogar) {
    ...
    <div className="min-h-screen text-white" style={{ background: "linear-gradient(180deg, #0f0f0f 0%, #1a1a1a 40%, #111111 100%)" }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5" style={{ background: "rgba(15,15,15,0.85)", backdropFilter: "blur(20px)" }}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src={bitcoperLogo} alt="Bitcopper Tech SpA" className="w-9 h-9 rounded-md object-contain" />
          ...
          <div className="flex items-center gap-3">
            <div className="relative" ref={clienteHogarRef}>
              <Button
                variant="outline"
                className="border-orange-500/30 bg-orange-500/10 text-orange-400 hover:bg-orange-500/20 hover:text-orange-300"
                onClick={() => setShowClienteHogar(!showClienteHogar)}
                data-testid="button-cliente-hogar"
              >
                <Home className="w-4 h-4 mr-2" />
                Cliente Hogar
              </Button>
              {showClienteHogar && (
                <div className="absolute right-0 top-full mt-3 w-[380px] rounded-xl border border-white/10 shadow-2xl p-6 z-50" style={{ background: "rgba(15,15,15,0.97)", backdropFilter: "blur(20px)" }}>
                  <div className="text-center mb-5">
                    <h3 className="text-xl font-bold text-white" data-testid="text-cliente-hogar-title">Construye con confianza</h3>
                    <p className="text-neutral-400 text-sm mt-1">Pagos protegidos para maestros y dueños de casa</p>
                  </div>
                  <div className="space-y-3">
                    <a href="/registro?role=maestro" className="block group" data-testid="link-role-maestro">
                      <div className="flex items-center gap-4 p-4 rounded-lg bg-orange-500/10 hover:bg-orange-500/20 transition-all cursor-pointer">
                        ...
                        <div className="text-left">
                          <p className="text-white font-semibold text-sm">Soy Maestro y quiero cobrar seguro</p>
                          <p className="text-neutral-500 text-xs mt-0.5">Recibe pagos protegidos por cada avance de obra</p>
                        </div>
                      </div>
                    </a>
                    <a href="/registro?role=homeowner" className="block group" data-testid="link-role-homeowner">
                      <div className="flex items-center gap-4 p-4 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 transition-all cursor-pointer">
                        ...
                        <div className="text-left">
                          <p className="text-white font-semibold text-sm">Soy Dueño de Casa y quiero proteger mi obra</p>
                          <p className="text-neutral-500 text-xs mt-0.5">Asegura tu inversión con pagos por hitos verificados</p>
                        </div>
                      </div>
                    </a>
                  </div>
      ...
      <section className="relative pt-32 pb-28 px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <p className="text-base text-neutral-500 max-w-xl mx-auto mb-10">
            De <span className="text-neutral-300 font-medium">Bitcopper Tech SpA</span> — Conecta tu Excel con Sodimac y Easy, calcula tu APU automáticamente y genera documentos con firma digital.
          </p>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-12 relative">
            ...
            <div className="text-center space-y-5 group">
              ...
              <h3 className="text-xl font-bold" data-testid="text-step-1">Carga tu Excel</h3>
      ...
      <footer className="py-12 px-6 relative">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <img src={bitcoperLogo} alt="Bitcopper Tech SpA" className="w-7 h-7 rounded-md object-contain" />
          ...
          <p className="text-sm text-neutral-500 text-center" data-testid="text-footer-copyright">
            &copy; 2026 SmartBuild APU Engine — Bitcopper Tech SpA. Todos los derechos reservados.
          </p>
          <div className="flex items-center gap-6 text-sm text-neutral-500">
            <a href="/registro" className="hover:text-white transition-colors" data-testid="link-registro">Registro Rápido</a>
            <a href="#como-funciona" className="hover:text-white transition-colors">Cómo Funciona</a>
            <a href="#beneficios" className="hover:text-white transition-colors">Beneficios</a>
            <a href="#precios" className="hover:text-white transition-colors">Precios</a>
          </div>
...
function SalesChatbot() {
  ...
  const [messages, setMessages] = useState<{ from: "bot" | "user"; text: string }[]>([
    { from: "bot", text: "¡Hola! Soy el asistente virtual de Bitcopper Tech SpA. ¿En qué puedo ayudarte?" }
  ]);
    ...
    setTimeout(() => {
      if (emailCollected) {
        ...
        if (emailRegex.test(userMsg)) {
          ...
          setMessages((prev) => [...prev, { from: "bot", text: "¡Perfecto! Hemos registrado tu correo. Un ejecutivo de Bitcopper Tech SpA te contactará para activar tu demo de SmartBuild APU. ¡Gracias!" }]);
        } else {
          setMessages((prev) => [...prev, { from: "bot", text: "Hola, soy el asistente de Bitcopper Tech SpA. Déjanos tu correo y un ejecutivo te contactará para activar tu demo de SmartBuild APU." }]);
        }
      }
    }, 500);
  };
        ...
        <div className="fixed bottom-6 right-6 z-[90] w-80 sm:w-96 rounded-xl shadow-2xl flex flex-col overflow-hidden" style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.08)", height: "420px" }} data-testid="chatbot-panel">
          <div className="flex items-center justify-between px-4 py-3" style={{ background: "linear-gradient(135deg, #f97316, #ea580c)" }}>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-white">Asistente Bitcopper</span>
            </div>
