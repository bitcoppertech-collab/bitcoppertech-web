      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {isLoading ? (
            ...
            <Card key={project.id} data-testid={`card-project-${project.id}`} className="group hover:border-primary/50 transition-all duration-300 hover:shadow-xl hover:shadow-black/20 bg-card">
              <CardContent className="p-6 flex flex-col h-full">
                ...
                <div className="mb-auto">
                  <h3 className="font-bold text-xl mb-1 group-hover:text-primary transition-colors line-clamp-1" data-testid={`text-project-name-${project.id}`}>
                    {project.name}
                  </h3>
                ...
                <div className="pt-4 border-t border-border/50 mt-4 flex items-center justify-between">
                  ...
                  <Link href={`/projects/${project.id}`}>
                    <Button size="sm" className="bg-secondary hover:bg-primary hover:text-white text-foreground transition-colors group-hover:translate-x-1" data-testid={`button-open-project-${project.id}`}>
                      Open
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteTarget ? `¿Estás seguro que deseas eliminar "${deleteTarget.name}"? Esta acción eliminará todos los datos del proyecto incluyendo sus ítems de presupuesto. No se puede deshacer.` : ""}
            </DialogDescription>
          </DialogHeader>
