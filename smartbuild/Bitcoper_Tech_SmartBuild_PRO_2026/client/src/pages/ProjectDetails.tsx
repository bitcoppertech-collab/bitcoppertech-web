...
export default function ProjectDetails() {
  ...
  const [editingGG, setEditingGG] = useState(false);
  const [editingUtil, setEditingUtil] = useState(false);
  const [ggDraft, setGGDraft] = useState("");
  const [utilDraft, setUtilDraft] = useState("");

  const savePercent = (field: "gastosGeneralesPercent" | "utilidadPercent", value: string) => {
    const parsed = parseFloat(value);
    if (!isNaN(parsed) && parsed >= 0) {
      updateProject.mutate({ id, updates: { [field]: parsed.toString() } });
    }
  };
                  ...
                  >
                    ...
                    <p className="text-muted-foreground text-sm max-w-sm">
                      {uploadBudget.isPending 
                        ...
                        : "Arrastra tu archivo Excel (.xlsx) aquí para cargar las partidas automáticamente."
                      }
                    </p>
                    {uploadBudget.isPending && (
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              {editingGG ? (
                                <input
                                  ...
                                  onChange={(e) => setGGDraft(e.target.value)}
                                  onBlur={() => { savePercent("gastosGeneralesPercent", ggDraft); setEditingGG(false); }}
                                  onKeyDown={(e) => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); if (e.key === "Escape") setEditingGG(false); }}
                                />
                              ) : (
                                <button
                                  className="font-mono bg-secondary/50 hover:bg-secondary px-1.5 py-0.5 rounded cursor-pointer transition-colors"
                                  onClick={() => { setGGDraft(String(financials.excel.gastosGeneralesPercent || 0)); setEditingGG(true); }}
                                  title="Editar % Gastos Generales"
                                  data-testid="button-edit-gg"
                                >
                                  {financials.excel.gastosGeneralesPercent}%
                                </button>
                              )}
                            </div>
                            <div className="flex items-center gap-1.5">
                              ...
                              {editingUtil ? (
                                <input
                                  ...
                                  onBlur={() => { savePercent("utilidadPercent", utilDraft); setEditingUtil(false); }}
                                  onKeyDown={(e) => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); if (e.key === "Escape") setEditingUtil(false); }}
                                />
                              ) : (
                                <button
                                  className="font-mono bg-secondary/50 hover:bg-secondary px-1.5 py-0.5 rounded cursor-pointer transition-colors"
                                  onClick={() => { setUtilDraft(String(financials.excel.utilidadPercent || 0)); setEditingUtil(true); }}
                                  title="Editar % Utilidad"
                                  data-testid="button-edit-util"
                                >
                                  {financials.excel.utilidadPercent}%
                                </button>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="font-mono font-medium" data-testid="text-excel-neto">{formatCLP(financials.excel.netoMateriales)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Gastos Generales ({financials.excel.gastosGeneralesPercent}%)</span>
                                <span className="font-mono" data-testid="text-excel-gg">{formatCLP(financials.excel.gastosGeneralesAmount)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Utilidad ({financials.excel.utilidadPercent}%)</span>
                                <span className="font-mono" data-testid="text-excel-util">{formatCLP(financials.excel.utilidadAmount)}</span>
                              </div>
                              <div className="flex justify-between border-t border-dashed border-border/50 pt-1">
                                ...
                                <span className="font-mono text-xs">{formatCLP(financials.excel.netoMateriales + financials.excel.gastosGeneralesAmount + financials.excel.utilidadAmount)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">IVA ({financials.excel.ivaPercent}%)</span>
                                <span className="font-mono" data-testid="text-excel-iva">{formatCLP(financials.excel.ivaAmount)}</span>
                              </div>
                              <div className="flex justify-between border-t border-border pt-2">
                                <span className="font-bold text-foreground">TOTAL PROYECTO</span>
                                <span className="font-mono font-bold text-primary" data-testid="text-excel-total">{formatCLP(financials.excel.total)}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Costo Directo Neto</span>
                                <span className="font-mono font-medium" data-testid="text-real-neto">{formatCLP(financials.real.netoMateriales)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Gastos Generales ({financials.real.gastosGeneralesPercent}%)</span>
                                <span className="font-mono" data-testid="text-real-gg">{formatCLP(financials.real.gastosGeneralesAmount)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Utilidad ({financials.real.utilidadPercent}%)</span>
                                <span className="font-mono" data-testid="text-real-util">{formatCLP(financials.real.utilidadAmount)}</span>
                              </div>
                              <div className="flex justify-between border-t border-dashed border-border/50 pt-1">
                                <span className="text-muted-foreground text-xs italic">Subtotal Neto Facturable</span>
                                <span className="font-mono text-xs">{formatCLP(financials.real.netoMateriales + financials.real.gastosGeneralesAmount + financials.real.utilidadAmount)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">IVA ({financials.real.ivaPercent}%)</span>
                                <span className="font-mono" data-testid="text-real-iva">{formatCLP(financials.real.ivaAmount)}</span>
                              </div>
                              <div className="flex justify-between border-t border-border pt-2">
                                <span className="font-bold text-foreground">TOTAL PROYECTO</span>
                                <span className="font-mono font-bold text-emerald-500" data-testid="text-real-total">{formatCLP(financials.real.total)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
          ...
          <TabsContent value="comparison" className="space-y-6">
            {financials ? (
              <>
                ...
                <Card className={cn(
                  "border shadow-md",
                  financials.margenManiobra.status === 'favorable' 
                    ? "bg-emerald-950/20 border-emerald-900/50"
                    : financials.margenManiobra.status === 'riesgo'
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="text-center p-4 bg-secondary/30 rounded-xl">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{formatCLP(financials.excel.utilidadAmount)}</p>
                      </div>
                      <div className="text-center p-4 bg-secondary/30 rounded-xl">
                        <p className="text-xs text-muted-foreground uppercase mb-1">Utilidad Real</p>
                        <p className={cn(
                          "text-2xl font-bold font-mono",
                          financials.margenManiobra.utilidadRealPercent > financials.excel.utilidadPercent ? "text-emerald-500" :
                          financials.margenManiobra.utilidadRealPercent < financials.excel.utilidadPercent ? "text-rose-500" :
                          "text-foreground"
                        )} data-testid="text-util-real">
                          {financials.margenManiobra.utilidadRealPercent}%
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">
                          {financials.margenManiobra.deltaPercent > 0 ? "+" : ""}{financials.margenManiobra.deltaPercent}% vs original
                        </p>
                      </div>
                      <div className="text-center p-4 bg-secondary/30 rounded-xl">
                        ...
                        <p className={cn(
                          "text-2xl font-bold font-mono",
                          financials.margenManiobra.deltaAmount > 0 ? "text-emerald-500" :
                          financials.margenManiobra.deltaAmount < 0 ? "text-rose-500" :
                          "text-foreground"
                        )} data-testid="text-delta-amount">
                          {financials.margenManiobra.deltaAmount > 0 ? "+" : ""}{formatCLP(financials.margenManiobra.deltaAmount)}
                        </p>
                    ...
                    {financials.margenManiobra.status === 'riesgo' && (
                      <div className="mt-4 p-3 bg-rose-500/10 rounded-lg flex items-start gap-3">
                        ...
                        <p className="text-sm text-rose-300">
                          Los materiales en el mercado cuestan más de lo presupuestado. Se está consumiendo {Math.abs(financials.margenManiobra.deltaPercent)}% de tu utilidad original.
                        </p>
                      </div>
                    )}
                  </CardContent>
                      ...
                      <table className="w-full text-sm" data-testid="table-comparison">
                        ...
                        <tbody className="divide-y divide-border/50">
                          <tr className="hover:bg-secondary/20">
                            ...
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.excel.netoMateriales)}</td>
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.real.netoMateriales)}</td>
                            <td className={cn("px-6 py-4 text-right font-mono font-medium",
                              financials.real.netoMateriales < financials.excel.netoMateriales ? "text-emerald-500" :
                              financials.real.netoMateriales > financials.excel.netoMateriales ? "text-rose-500" : ""
                            )}>
                              {formatCLP(financials.real.netoMateriales - financials.excel.netoMateriales)}
                            </td>
                          </tr>
                          <tr className="hover:bg-secondary/20">
                            <td className="px-6 py-4 font-medium">Gastos Generales ({financials.excel.gastosGeneralesPercent}%)</td>
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.excel.gastosGeneralesAmount)}</td>
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.real.gastosGeneralesAmount)}</td>
                            <td className={cn("px-6 py-4 text-right font-mono font-medium",
                              financials.real.gastosGeneralesAmount < financials.excel.gastosGeneralesAmount ? "text-emerald-500" :
                              financials.real.gastosGeneralesAmount > financials.excel.gastosGeneralesAmount ? "text-rose-500" : ""
                            )}>
                              {formatCLP(financials.real.gastosGeneralesAmount - financials.excel.gastosGeneralesAmount)}
                            </td>
                          </tr>
                          <tr className="hover:bg-secondary/20">
                            <td className="px-6 py-4 font-medium">Utilidad ({financials.excel.utilidadPercent}%)</td>
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.excel.utilidadAmount)}</td>
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.real.utilidadAmount)}</td>
                            <td className={cn("px-6 py-4 text-right font-mono font-medium",
                              financials.real.utilidadAmount < financials.excel.utilidadAmount ? "text-emerald-500" :
                              financials.real.utilidadAmount > financials.excel.utilidadAmount ? "text-rose-500" : ""
                            )}>
                              {formatCLP(financials.real.utilidadAmount - financials.excel.utilidadAmount)}
                            </td>
                          </tr>
                          <tr className="hover:bg-secondary/20">
                            <td className="px-6 py-4 font-medium">IVA ({financials.excel.ivaPercent}%)</td>
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.excel.ivaAmount)}</td>
                            <td className="px-6 py-4 text-right font-mono">{formatCLP(financials.real.ivaAmount)}</td>
                            <td className={cn("px-6 py-4 text-right font-mono font-medium",
                              financials.real.ivaAmount < financials.excel.ivaAmount ? "text-emerald-500" :
                              financials.real.ivaAmount > financials.excel.ivaAmount ? "text-rose-500" : ""
                            )}>
                              {formatCLP(financials.real.ivaAmount - financials.excel.ivaAmount)}
                            </td>
                          </tr>
                          <tr className="bg-secondary/30 font-bold">
                            <td className="px-6 py-4">TOTAL</td>
                            <td className="px-6 py-4 text-right font-mono text-amber-500">{formatCLP(financials.excel.total)}</td>
                            <td className="px-6 py-4 text-right font-mono text-emerald-500">{formatCLP(financials.real.total)}</td>
                            <td className={cn("px-6 py-4 text-right font-mono",
                              financials.real.total < financials.excel.total ? "text-emerald-500" :
                              financials.real.total > financials.excel.total ? "text-rose-500" : ""
                            )}>
                              {formatCLP(financials.real.total - financials.excel.total)}
                            </td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-foreground" data-testid="text-savings">
                        {financials && financials.margenManiobra.deltaAmount > 0 
                          ? formatCLP(financials.margenManiobra.deltaAmount) 
                          : formatCLP(0)}
                      </h3>
