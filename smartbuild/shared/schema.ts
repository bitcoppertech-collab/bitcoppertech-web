...
export const companySettings = pgTable("company_settings", {
  ...
  defaultGGPercent: decimal("default_gg_percent", { precision: 5, scale: 2 }).default("15"),
  defaultUtilidadPercent: decimal("default_utilidad_percent", { precision: 5, scale: 2 }).default("10"),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
// === BASE SCHEMAS ===
export const insertProjectSchema = createInsertSchema(projects).omit({ id: true, ownerId: true, createdAt: true, updatedAt: true });
...
// Admin Transactions (Bitcopper business layer)
export const adminTransactions = pgTable("admin_transactions", {
  ...
  pagoRetail: decimal("pago_retail", { precision: 15, scale: 2 }).notNull(),
  comisionBitcopper: decimal("comision_bitcopper", { precision: 15, scale: 2 }).notNull(),
  estado: text("estado").default("Pendiente"),
  createdAt: timestamp("created_at").defaultNow(),
});

export const insertAdminTransactionSchema = createInsertSchema(adminTransactions).omit({ id: true, createdAt: true });
export type AdminTransaction = typeof adminTransactions.$inferSelect;
export type InsertAdminTransaction = z.infer<typeof insertAdminTransactionSchema>;
