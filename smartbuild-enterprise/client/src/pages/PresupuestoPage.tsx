  ...
  return (
    <div style={{ borderTop: "1px solid rgba(255,255,255,0.04)", background: "#0F1C28" }}>
      {Object.entries(grupos).map(([tipo, items]) => {
        ...
        const subtotal = items.reduce((s, i) => s + parseFloat(i.totalUF ?? i.totalUf ?? i.total_uf ?? "0"), 0);
        return (
            ...
            {items.map((item, i) => (
              <div key={i} style={{
                ...
                <span style={{ textAlign: "right", color: "#C17F3A" }}>
                  {fUF(parseFloat(item.totalUF ?? item.totalUf ?? item.total_uf ?? "0"))}
                </span>
              </div>
            ))}
          </div>
        ...
        <div>
          <div style={{
            ...
            <span style={{ marginLeft: "auto", color: "#6A7A8A", fontWeight: 400 }}>
              {fUF(subcontratos.reduce((s: number, sc: any) => s + parseFloat(sc.montoUF ?? sc.montoUf ?? sc.monto_uf ?? "0"), 0))}
            </span>
          </div>
