...
import { Plus, FolderOpen, HardHat, Box, ArrowRight, X } from "lucide-react";

function NewProjectModal({ onClose }: { onClose: () => void }) {
  const qc = useQueryClient();
  const [form, setForm] = useState({ name: "", client: "", location: "", description: "", totalBudget: "" });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="bg-[#1C2B3A] border border-[rgba(193,127,58,0.3)] w-full max-w-md rounded-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[rgba(193,127,58,0.12)]">
        ...
        <div className="p-6 space-y-4">
          {[
            { key: "name", label: "Nombre del proyecto *", placeholder: "Ej: Edificio Torres del Sur" },
            { key: "client", label: "Cliente", placeholder: "Ej: Constructora X" },
            { key: "location", label: "Ubicación", placeholder: "Ej: Calama, Antofagasta" },
            { key: "totalBudget", label: "Presupuesto total (UF)", placeholder: "Ej: 15000", type: "number" },
          ].map(({ key, label, placeholder, type }) => (
            <div key={key}>
              <label className="block text-[10px] font-mono text-[#6A7A8A] uppercase tracking-widest mb-1.5">{label}</label>
              <input type={type || "text"} placeholder={placeholder}
                value={(form as any)[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                className="w-full bg-[rgba(255,255,255,0.04)] border border-[rgba(193,127,58,0.2)] text-white px-4 py-2.5 text-sm rounded-sm outline-none focus:border-[#C17F3A] transition-colors" />
            </div>
          ))}
          <div>
            <label className="block text-[10px] font-mono text-[#6A7A8A] uppercase tracking-widest mb-1.5">Descripción</label>
            <textarea placeholder="Descripción del proyecto..." rows={3}
              value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
              className="w-full bg-[rgba(255,255,255,0.04)] border border-[rgba(193,127,58,0.2)] text-white px-4 py-2.5 text-sm rounded-sm outline-none focus:border-[#C17F3A] transition-colors resize-none" />
          </div>
        </div>
        <div className="px-6 pb-6 flex gap-3">
          <button onClick={onClose} className="flex-1 border border-[rgba(193,127,58,0.2)] text-[#6A7A8A] hover:text-white py-2.5 text-sm rounded-sm transition-colors">Cancelar</button>
              ...
              <div key={p.id} className="bg-[#1C2B3A] border border-[rgba(193,127,58,0.1)] p-5 hover:border-[rgba(193,127,58,0.3)] transition-colors">
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-[#6A7A8A]">{p.client ?? "Sin cliente"}{p.location ? ` · ${p.location}` : ""}</div>
